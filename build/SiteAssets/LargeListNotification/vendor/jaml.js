    ...
    Jaml.Node.prototype = {
      ...
      */
        render: function(lpad) {
            ...
            if (!textnode)
                node.push(this.getPadding(lpad));
      ...
      */
        multiLineTag: function() {
            ...
            if (childLength == 1 && this.children[0]instanceof Jaml.TextNode)
                multiLine = false;
      ...
      */
        isSelfClosing: function() {
            for (var i = this.notSelfClosingTags.length - 1; i >= 0; i--) {
                if (this.tagName == this.notSelfClosingTags[i])
                    return false;
                }
      ...
      */
        var makeTagHelper = function(tagName) {
            return function(attrs) {
                ...
                var firstArgIsAttributes = (typeof attrs == 'object') && !(attrs instanceof Jaml.Node) && !(attrs instanceof Jaml.TextNode);

                if (firstArgIsAttributes)
                    node.setAttributes(attrs);

                var startIndex = firstArgIsAttributes
                    ? 1
                    : 0;
